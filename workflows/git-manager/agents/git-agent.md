...
model: sonnet
disallowed-tools:
  # env / config / generic secrets (family globs cover .env, .env.*, *.env, .envrc, .env.vault)
  - Read(**/*.env*)
  - Read(**/auth.json)
  - Read(**/*credentials*)
  - Read(**/*secrets.*)
  ...
  # private keys / keystores (*.key covers config/master.key and config/credentials/*.key)
  - Read(**/*.pem)
  - Read(**/*.key)
  - Read(**/*.p12)
  - Read(**/*.pfx)
  ...
  - Read(**/id_ecdsa*)
  - Read(**/id_dsa*)
  # host / registry / cloud (recursive globs cover nested credentials and configs)
  - Read(**/.ssh/**)
  - Read(**/.netrc)
  ...
  - Grep(path=**/*.pem)
  - Grep(path=**/*.key)
  # cat / less / head / tail blocked broadly below in shell-escape section — no need to enumerate per-secret variants
  - Bash(source *.env*)
  - Bash(. *.env*)
  - Bash(env)
  - Bash(printenv*)
